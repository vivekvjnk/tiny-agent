...
```
 ...
 │    Init     │     storage controller (eMMC / SPI Flash / PCIe).
 └─────────────┘
      │
      ▼
 ┌─────────────┐
 │  DDR PHY    │ ──> Executes [DQS read/write leveling](Memory.md#2-dqs-write-leveling-time-domain-training), setup/hold adjustments, and 
 │ Calibration │     VREF tuning to make main DRAM operational.
 └─────────────┘
      │
      ▼
 ┌─────────────┐
 │ Authenticate│ ──> Verifies cryptographic signatures of BL31, BL32, and BL33 binaries 
 │   & Load    │     against hardware Root of Trust. Maps binaries into DRAM.
 └─────────────┘
      │
      ▼
 [BL31 Handoff]
